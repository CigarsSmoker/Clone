µMatrix 和 [µBlock](https://github.com/gorhill/uBlock) 都是从 [HTTP Switchboard] https://github.com/gorhill/httpswitchboard) ("HTTPSB") 的分支出来的。它们都在 HTTPSB 的基础上做出了显著的改进。简单的说 HTTPSB 是漂亮的原型，是测试各种创意的概念样本。µMatrix 和 µBlock 就是最终的产品。

µMatrix 继承了基于矩阵过滤的任务，而 µBlock 则继承了基于模本过滤的任务。

µMatrix 和 HTTPSB 比较的各种差异解释如下。
